...
for enterprise_value_to_revenue_limit in range(MIN_EVR,MAX_EVR+1):
    for profit_margin_limit in range(PM_MIN,PM_MAX+1):
        # TASE:
        # =====
        num_results = sss.sss_run(sectors_list=[], build_csv_db_only=0, build_csv_db=0, csv_db_path='Results/20201115-145013_TASE_FAVOR_TECH_BY3_MARKETCAP_PMARGIN0.0567_EVR15.0_BUILD_DB_ONLY_NUM_RESULTS_443', read_united_states_input_symbols=0, tase_mode=1, num_threads=1, forward_eps_included=0, market_cap_included=1, use_investpy=0, research_mode=1, profit_margin_limit=float(profit_margin_limit)/100.0, best_n_select=3, enterprise_value_to_revenue_limit=enterprise_value_to_revenue_limit)
        mode_str    = 'TASE_'
        ...
        # mode_str    = 'NASDAQ100_RUSSEL1000_SNP500_'
