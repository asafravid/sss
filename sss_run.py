        ...
        if run_all:
            if not sss_config.aggregate_only:
                for db_filename in DB_FILENAMES:
                    ...
                    ev_range_all          = get_range(csv_db_path=new_run_all, db_filename=db_filename, column_name='enterprise_value',         num_sections=1 if sss_config.custom_sss_value_equation else 5, reverse=0, pop_1st_percentile_range=False if sss_config.custom_sss_value_equation else True)
                    pe_range_all          = get_range(csv_db_path=new_run_all, db_filename=db_filename, column_name='pe_effective',             num_sections=1 if sss_config.custom_sss_value_equation else 7, reverse=1, pop_1st_percentile_range=False if sss_config.custom_sss_value_equation else True)
                    evr_range_all         = get_range(csv_db_path=new_run_all, db_filename=db_filename, column_name='evr_effective',            num_sections=1 if sss_config.custom_sss_value_equation else 8, reverse=1, pop_1st_percentile_range=False if sss_config.custom_sss_value_equation else True)
                    pm_ratios_range_all   = get_range(csv_db_path=new_run_all, db_filename=db_filename, column_name='effective_profit_margin',  num_sections=1 if sss_config.custom_sss_value_equation else 9, reverse=0, pop_1st_percentile_range=False if sss_config.custom_sss_value_equation else True)
