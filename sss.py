...
#             7. Add TTM (Forward caculations), forwardPE
#             8. Add Free Cash flow [FCF] (EV/FreeCashFlow): Inverse of the Free Cash Flow Yield (https://www.stockopedia.com/ratios/ev-free-cash-flow-336/#:~:text=What%20is%20the%20definition%20of,the%20Free%20Cash%20Flow%20Yield.)
#             8.1. There is already an EV/CFO ratio.
...
# TODO: ASAFR: For the quarterly_profit_margin: 1. Damp more (0.01), 2. Distribute weight differently. 3. So that there are less filtering.
#                                               4. But first - compare to last week's results - any major differences? If not - then all of this is in lower priority
...
#              2. Add Forward PE (less weight)
#              3. Lower Market Cap - Give more weight (in multi-dimentional scan)
    ...
    try:
        ...
        if build_csv_db:
            try:
                ...
                cash_flows                             = symbol.get_cashflow(     as_dict=True, freq="yearly")  # TODO: ASAFR: As In here, add the quarterly data, and process it as well
                ...
                if stock_data.trailing_12months_price_to_sales is not None and stock_data.trailing_12months_price_to_sales > 0 and stock_data.effective_profit_margin is not None and stock_data.effective_profit_margin > 0 and earnings_qgrowth_factor_effective is not None and earnings_qgrowth_factor_effective > 0 and stock_data.price_to_earnings_to_growth_ratio is not None and stock_data.price_to_earnings_to_growth_ratio > 0 and stock_data.pe_effective is not None and stock_data.pe_effective > 0 and stock_data.enterprise_value_to_ebitda is not None and stock_data.enterprise_value_to_ebitda > 0 and stock_data.ev_to_cfo_ratio_effective is not None and stock_data.ev_to_cfo_ratio_effective > 0 and stock_data.sqrt_peg_ratio is not None and stock_data.sqrt_peg_ratio > 0:
                    if stock_data.price_to_book is not None and stock_data.price_to_book > 0:
                        if stock_data.debt_to_equity_effective > 0:
                            ...
                            stock_data.sss_value   = float(((stock_data.evr_effective * stock_data.pe_effective * stock_data.enterprise_value_to_ebitda * stock_data.trailing_12months_price_to_sales * stock_data.price_to_book) / (stock_data.effective_profit_margin                                       )) * ((stock_data.sqrt_peg_ratio * stock_data.ev_to_cfo_ratio_effective * effective_debt_to_equity_effective) / earnings_qgrowth_factor_effective))  # The lower  the better
                            stock_data.sssi_value  = float(((stock_data.evr_effective * stock_data.pe_effective * stock_data.enterprise_value_to_ebitda * stock_data.trailing_12months_price_to_sales * stock_data.price_to_book) / (stock_data.effective_profit_margin * stock_data.held_percent_institutions)) * ((stock_data.sqrt_peg_ratio * stock_data.ev_to_cfo_ratio_effective * effective_debt_to_equity_effective) / earnings_qgrowth_factor_effective))  # The lower  the better
                            stock_data.ssss_value  = float(((stock_data.evr_effective * stock_data.pe_effective * stock_data.enterprise_value_to_ebitda * stock_data.trailing_12months_price_to_sales * stock_data.price_to_book) / (stock_data.effective_profit_margin                                       )) * ((stock_data.sqrt_peg_ratio * stock_data.ev_to_cfo_ratio_effective * effective_debt_to_equity_effective) / earnings_qgrowth_factor_effective))  # the lower  the better
                            stock_data.ssssi_value = float(((stock_data.evr_effective * stock_data.pe_effective * stock_data.enterprise_value_to_ebitda * stock_data.trailing_12months_price_to_sales * stock_data.price_to_book) / (stock_data.effective_profit_margin * stock_data.held_percent_institutions)) * ((stock_data.sqrt_peg_ratio * stock_data.ev_to_cfo_ratio_effective * effective_debt_to_equity_effective) / earnings_qgrowth_factor_effective))  # the lower  the better
                        stock_data.sssss_value     = float(((stock_data.evr_effective * stock_data.pe_effective * stock_data.enterprise_value_to_ebitda * stock_data.trailing_12months_price_to_sales * stock_data.price_to_book) / (stock_data.effective_profit_margin                                       )) * ((stock_data.sqrt_peg_ratio * stock_data.ev_to_cfo_ratio_effective                                     ) / earnings_qgrowth_factor_effective))  # the lower  the better
                        stock_data.sssssi_value    = float(((stock_data.evr_effective * stock_data.pe_effective * stock_data.enterprise_value_to_ebitda * stock_data.trailing_12months_price_to_sales * stock_data.price_to_book) / (stock_data.effective_profit_margin * stock_data.held_percent_institutions)) * ((stock_data.sqrt_peg_ratio * stock_data.ev_to_cfo_ratio_effective                                     ) / earnings_qgrowth_factor_effective))  # the lower  the better
    ...
    if build_csv_db:
        for symb in symbols:
            ...
            if not process_info(symbol=symbol, stock_data=stock_data, build_csv_db_only=build_csv_db_only, use_investpy=use_investpy, tase_mode=tase_mode, sectors_list=sectors_list, sectors_filter_out=sectors_filter_out, countries_list=countries_list, countries_filter_out=countries_filter_out, build_csv_db=build_csv_db, profit_margin_limit=profit_margin_limit, ev_to_cfo_ratio_limit=ev_to_cfo_ratio_limit, debt_to_equity_limit=debt_to_equity_limit, min_enterprise_value_millions_usd=min_enterprise_value_millions_usd, earnings_quarterly_growth_min=earnings_quarterly_growth_min, revenue_quarterly_growth_min=revenue_quarterly_growth_min, enterprise_value_to_revenue_limit=enterprise_value_to_revenue_limit, favor_sectors=favor_sectors, favor_sectors_by=favor_sectors_by, market_cap_included=market_cap_included, research_mode=research_mode, currency_conversion_tool=currency_conversion_tool):
                ...
                #                              Ticker	          Name	                 Sector	            Country             sss_value	          ssss_value	         sssss_value	         ssse_value	            sssse_value	            ssssse_value	         sssi_value	            ssssi_value	            sssssi_value	         sssei_value	         ssssei_value	          sssssei_value	 annualized_revenue             annualized_earnings             quarterized_revenue             quarterized_earnings,            effective_earnings,            effective_revenue,            enterprise_value_to_revenue             evr_effective             trailing_price_to_earnings             forward_price_to_earnings             effective_price_to_earnings             trailing_12months_price_to_sales                        pe_effective	            enterprise_value_to_ebitda	           profit_margin	         annualized_profit_margin	          annualized_profit_margin_boost             quarterized_profit_margin             quarterized_profit_margin_boost             effective_profit_margin             held_percent_institutions             forward_eps             trailing_eps             previous_close             trailing_eps_percentage             price_to_book             shares_outstanding             net_income_to_common_shareholders             nitcsh_to_shares_outstanding             num_employees             enterprise_value             market_cap             nitcsh_to_num_employees             earnings_quarterly_growth             revenue_quarterly_growth             price_to_earnings_to_growth_ratio             sqrt_peg_ratio             annualized_cash_flow_from_operating_activities             quarterized_cash_flow_from_operating_activities             annualized_ev_to_cfo_ratio             quarterized_ev_to_cfo_ratio             ev_to_cfo_ratio_effective             annualized_total_liab             annualized_total_stockholder_equity             annualized_debt_to_equity             quarterized_total_liab             quarterized_total_stockholder_equity             quarterized_debt_to_equity             debt_to_equity_effective             financial_currency             conversion_rate_mult_to_usd             last_dividend_0                last_dividend_1                last_dividend_2                last_dividend_3
                csv_db_data.append([stock_data.ticker, stock_data.short_name, stock_data.sector, stock_data.country, stock_data.sss_value, stock_data.ssss_value, stock_data.sssss_value, stock_data.ssse_value, stock_data.sssse_value, stock_data.ssssse_value, stock_data.sssi_value, stock_data.ssssi_value, stock_data.sssssi_value, stock_data.sssei_value, stock_data.ssssei_value, stock_data.sssssei_value, stock_data.annualized_revenue, stock_data.annualized_earnings, stock_data.quarterized_revenue, stock_data.quarterized_earnings, stock_data.effective_earnings, stock_data.effective_revenue, stock_data.enterprise_value_to_revenue, stock_data.evr_effective, stock_data.trailing_price_to_earnings, stock_data.forward_price_to_earnings, stock_data.effective_price_to_earnings, stock_data.trailing_12months_price_to_sales, stock_data.pe_effective, stock_data.enterprise_value_to_ebitda, stock_data.profit_margin, stock_data.annualized_profit_margin, stock_data.annualized_profit_margin_boost, stock_data.quarterized_profit_margin, stock_data.quarterized_profit_margin_boost, stock_data.effective_profit_margin, stock_data.held_percent_institutions, stock_data.forward_eps, stock_data.trailing_eps, stock_data.previous_close, stock_data.trailing_eps_percentage, stock_data.price_to_book, stock_data.shares_outstanding, stock_data.net_income_to_common_shareholders, stock_data.nitcsh_to_shares_outstanding, stock_data.num_employees, stock_data.enterprise_value, stock_data.market_cap, stock_data.nitcsh_to_num_employees, stock_data.earnings_quarterly_growth, stock_data.revenue_quarterly_growth, stock_data.price_to_earnings_to_growth_ratio, stock_data.sqrt_peg_ratio, stock_data.annualized_cash_flow_from_operating_activities, stock_data.quarterized_cash_flow_from_operating_activities, stock_data.annualized_ev_to_cfo_ratio, stock_data.quarterized_ev_to_cfo_ratio, stock_data.ev_to_cfo_ratio_effective, stock_data.annualized_total_liab, stock_data.annualized_total_stockholder_equity, stock_data.annualized_debt_to_equity, stock_data.quarterized_total_liab, stock_data.quarterized_total_stockholder_equity, stock_data.quarterized_debt_to_equity, stock_data.debt_to_equity_effective, stock_data.financial_currency, stock_data.conversion_rate_mult_to_usd, stock_data.last_4_dividends_0, stock_data.last_4_dividends_1, stock_data.last_4_dividends_2, stock_data.last_4_dividends_3])
                continue
