...
def process_info(symbol, stock_data, build_csv_db_only, use_investpy, tase_mode, sectors_list, sectors_filter_out, countries_list, countries_filter_out, build_csv_db, profit_margin_limit, ev_to_cfo_ratio_limit, debt_to_equity_limit, min_enterprise_value_millions_usd, earnings_quarterly_growth_min, revenue_quarterly_growth_min, enterprise_value_to_revenue_limit, favor_sectors, favor_sectors_by, market_cap_included, research_mode, currency_conversion_tool):
    try:
        ...
        # TODO: ASAFR: Add the quarterized_profit_margin limit here
        ...
        if build_csv_db:
            if return_value:
                ...
                if stock_data.trailing_12months_price_to_sales is not None and stock_data.trailing_12months_price_to_sales > 0 and max_profit_margin_effective is not None and max_profit_margin_effective > 0 and earnings_qgrowth_factor_effective is not None and earnings_qgrowth_factor_effective > 0 and stock_data.price_to_earnings_to_growth_ratio is not None and stock_data.price_to_earnings_to_growth_ratio > 0 and stock_data.tpe_effective is not None and stock_data.tpe_effective > 0 and stock_data.enterprise_value_to_ebitda is not None and stock_data.enterprise_value_to_ebitda > 0 and stock_data.ev_to_cfo_ratio is not None and stock_data.ev_to_cfo_ratio > 0 and stock_data.sqrt_peg_ratio is not None and stock_data.sqrt_peg_ratio > 0:
                    ...
                    stock_data.sssss_value        = float(((stock_data.evr_effective * stock_data.tpe_effective * stock_data.enterprise_value_to_ebitda * stock_data.trailing_12months_price_to_sales                           ) / (max_profit_margin_effective                                       )) * ((stock_data.sqrt_peg_ratio * stock_data.ev_to_cfo_ratio                                      ) / earnings_qgrowth_factor_effective))  # the lower  the better
