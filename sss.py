            ...
            try:
                for key in reversed(list(cash_flows)):  # The 1st element will be the oldest, receiving the lowest weight
                    if 'Total Cash From Operating Activities' in cash_flows[key] and not math.isnan(cash_flows[key]['Total Cash From Operating Activities']):
                        ...
                        weight_index += 1
                stock_data.annualized_cash_flow_from_operating_activities = sum(cash_flows_list) / weights_sum
            except Exception as e:
                pass

            # if 'Total Cash From Operating Activities' in cash_flow[list(cash_flow)[0]]:
            #     stock_data.annualized_cash_flow_from_operating_activities = cash_flow[list(cash_flow)[0]]['Total Cash From Operating Activities']
