#############################################################################
#
# Version 313 - Author: Asaf Ravid <asaf.rvd@gmail.com>
#
    ...
    try:
        ...
        if build_csv_db:
            ...
            if 'priceToSalesTrailing12Months' in info and info['priceToSalesTrailing12Months'] is not None:
                ...
                if stock_data.annualized_revenue is not None and stock_data.annualized_revenue > 0:  # TODO: ASAFR: Take quarterized_revenue here? Investigate
                    stock_data.trailing_12months_price_to_sales  = stock_data.market_cap / stock_data.annualized_revenue  # annualized_revenue is already in USD (converted earlier)
                else:
        ...
        #         return_value = False
            ...
            # if no enterprise_value_to_ebitda, use earnings. TODO: ASAFR: Which earnings is better - annual or quarterly or some average of them? Investigate: does 4 quarters equal the annual?
            if (stock_data.enterprise_value_to_ebitda is None or stock_data.enterprise_value_to_ebitda < 0) and stock_data.annualized_earnings > 0:
                stock_data.enterprise_value_to_ebitda = float(stock_data.enterprise_value) / stock_data.annualized_earnings  #  annualized_earnings is converted already to USD
        ...
        if build_csv_db_only:
            if (stock_data.enterprise_value_to_revenue is not None and stock_data.enterprise_value_to_revenue <= 0 or stock_data.enterprise_value_to_revenue is None) and stock_data.annualized_revenue > 0:
                stock_data.evr_effective = stock_data.enterprise_value/stock_data.annualized_revenue  # TODO: ASAFR: What about the quarterly value?
            else:
        ...
        if not build_csv_db_only and (stock_data.trailing_price_to_earnings is None or stock_data.trailing_price_to_earnings <= 0):
            if return_value and (not research_mode or VERBOSE_LOGS): print('                            Skipping {} trailing_price_to_earnings: {}'.format(stock_data.ticker, stock_data.trailing_price_to_earnings))
            return_value = False
...
def process_symbols(symbols, csv_db_data, rows, rows_no_div, rows_only_div, thread_id, build_csv_db_only, use_investpy, tase_mode, sectors_list, sectors_filter_out, countries_list, countries_filter_out, build_csv_db, relaxed_access, profit_margin_limit, ev_to_cfo_ratio_limit, debt_to_equity_limit, min_enterprise_value_millions_usd, earnings_quarterly_growth_min, revenue_quarterly_growth_min, enterprise_value_to_revenue_limit, favor_sectors, favor_sectors_by, market_cap_included, research_mode, currency_conversion_tool):
    ...
    if build_csv_db:
        ...
        for row in csv_db_data:
            ...
            stock_data = StockData(ticker=symbol, short_name=row[1], sector=row[2], country=row[3], sss_value=float(row[4]), ssss_value=float(row[5]), sssss_value=float(row[6]), ssse_value=float(row[7]), sssse_value=float(row[8]), ssssse_value=float(row[9]), sssi_value=float(row[10]), ssssi_value=float(row[11]), sssssi_value=float(row[12]), sssei_value=float(row[13]), ssssei_value=float(row[14]), sssssei_value=float(row[15]), annualized_revenue=float(row[16]), annualized_earnings=float(row[17]), quarterized_revenue=float(row[18]), quarterized_earnings=float(row[19]), enterprise_value_to_revenue=float(row[20]), evr_effective=float(row[21]), trailing_price_to_earnings=float(row[22]), trailing_12months_price_to_sales=float(row[23]), tpe_effective=float(row[24]), enterprise_value_to_ebitda=float(row[25]), profit_margin=float(row[26]), annualized_profit_margin=float(row[27]), annualized_profit_margin_boost=float(row[28]), quarterized_profit_margin=float(row[29]), quarterized_profit_margin_boost=float(row[30]), effective_profit_margin=float(row[31]), held_percent_institutions=float(row[32]), forward_eps=float(row[33]), trailing_eps=float(row[34]), previous_close=float(row[35]), trailing_eps_percentage=float(row[36]), price_to_book=float(row[37]), shares_outstanding=float(row[38]), net_income_to_common_shareholders=float(row[39]), nitcsh_to_shares_outstanding=float(row[40]), num_employees=int(float(row[41])), enterprise_value=int(float(row[42])), market_cap=int(float(row[43])), nitcsh_to_num_employees=float(row[44]), earnings_quarterly_growth=float(row[45]), revenue_quarterly_growth=float(row[46]), price_to_earnings_to_growth_ratio=float(row[47]), sqrt_peg_ratio=float(row[48]), annualized_cash_flow_from_operating_activities=float(row[49]), quarterized_cash_flow_from_operating_activities=float(row[50]), annualized_ev_to_cfo_ratio=float(row[51]), quarterized_ev_to_cfo_ratio=float(row[52]), ev_to_cfo_ratio_effective=float(row[53]), annualized_total_liab=float(row[54]), annualized_total_stockholder_equity=float(row[55]), annualized_debt_to_equity=float(row[56]), quarterized_total_liab=float(row[57]), quarterized_total_stockholder_equity=float(row[58]), quarterized_debt_to_equity=float(row[59]), debt_to_equity_effective=float(row[60]), financial_currency=row[61], conversion_rate_mult_to_usd=float(row[62]), last_4_dividends_0=float(row[63]), last_4_dividends_1=float(row[64]), last_4_dividends_2=float(row[65]), last_4_dividends_3=float(row[66]))
