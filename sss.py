    ...
    try:
        ...
        if build_csv_db:
            ...
            if 'priceToSalesTrailing12Months' in info and info['priceToSalesTrailing12Months'] is not None:
                ...
                if stock_data.annualized_revenue is not None and stock_data.annualized_revenue > 0:
...
def process_symbols(symbols, csv_db_data, rows, rows_no_div, rows_only_div, thread_id, build_csv_db_only, use_investpy, tase_mode, sectors_list, sectors_filter_out, countries_list, countries_filter_out, build_csv_db, relaxed_access, profit_margin_limit, ev_to_cfo_ratio_limit, debt_to_equity_limit, min_enterprise_value_millions_usd, earnings_quarterly_growth_min, revenue_quarterly_growth_min, enterprise_value_to_revenue_limit, favor_sectors, favor_sectors_by, market_cap_included, research_mode, currency_conversion_tool):
    ...
    if build_csv_db:
        ...
        for row in csv_db_data:
            ...
            stock_data = StockData(ticker=symbol, short_name=row[1], sector=row[2], country=row[3], sss_value=float(row[4]), ssss_value=float(row[5]), sssss_value=float(row[6]), ssse_value=float(row[7]), sssse_value=float(row[8]), ssssse_value=float(row[9]), sssi_value=float(row[10]), ssssi_value=float(row[11]), sssssi_value=float(row[12]), sssei_value=float(row[13]), ssssei_value=float(row[14]), sssssei_value=float(row[15]), annualized_revenue=float(row[16]), annualized_earnings=float(row[17]), enterprise_value_to_revenue=float(row[18]), evr_effective=float(row[19]), trailing_price_to_earnings=float(row[20]), trailing_12months_price_to_sales=float(row[21]), tpe_effective=float(row[22]), enterprise_value_to_ebitda=float(row[23]), profit_margin=float(row[24]), annualized_profit_margin=float(row[25]), annualized_profit_margin_boost=float(row[26]), held_percent_institutions=float(row[27]), forward_eps=float(row[28]), trailing_eps=float(row[29]), previous_close=float(row[30]), trailing_eps_percentage=float(row[31]), price_to_book=float(row[32]), shares_outstanding=float(row[33]), net_income_to_common_shareholders=float(row[34]), nitcsh_to_shares_outstanding=float(row[35]), num_employees=int(float(row[36])), enterprise_value=int(float(row[37])), market_cap=int(float(row[38])), nitcsh_to_num_employees=float(row[39]), earnings_quarterly_growth=float(row[40]), revenue_quarterly_growth=float(row[41]), price_to_earnings_to_growth_ratio=float(row[42]), sqrt_peg_ratio=float(row[43]), annualized_cash_flow_from_operating_activities=float(row[44]), ev_to_cfo_ratio=float(row[45]), annualized_total_liab=float(row[46]), annualized_total_stockholder_equity=float(row[47]), annualized_debt_to_equity=float(row[48]), financial_currency=row[49], conversion_rate_mult_to_usd=float(row[50]), last_4_dividends_0=float(row[51]), last_4_dividends_1=float(row[52]), last_4_dividends_2=float(row[53]), last_4_dividends_3=float(row[54]))
