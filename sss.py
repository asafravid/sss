            ...
            try:
                if reference_raw_data != None:
                    if      symbol_data['MA21exp'].iloc[-1] > symbol_data['MA21exp'].iloc[-2] > symbol_data['MA21exp'].iloc[-3] and \
                        ...
                        print('[perform_scan_close_values_days] processing rising {:11} ({:40}). ({:4}/{:4} [{:5}%]) total_free_mem_mb={:5}'.format(symbol, symbol_name, len(rising_symbols)+1, len(symbols), 100*(len(rising_symbols)+1)/(len(symbols)), total_free_mem_mb))
