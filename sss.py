            ...
            for fix_row_index in range(3,len(row)):  # for empty strings - convert value to 0
                if row[fix_row_index] == '':
                    if fix_row_index == 1:  # Name == '' --> 'None'
                        ...
                        row[fix_row_index] = 0
            stock_data = StockData(ticker=symbol, short_name=row[1], sector=row[2], sss_value=float(row[3]), ssss_value=float(row[4]), sssss_value=float(row[5]), ssse_value=float(row[6]), sssse_value=float(row[7]), ssssse_value=float(row[8]), sssi_value=float(row[9]), ssssi_value=float(row[10]), sssssi_value=float(row[11]), sssei_value=float(row[12]), ssssei_value=float(row[13]), sssssei_value=float(row[14]), enterprise_value_to_revenue=float(row[15]), evr_effective=float(row[16]), trailing_price_to_earnings=float(row[17]), trailing_12months_price_to_sales=float(row[18]), tpe_effective=float(row[19]), enterprise_value_to_ebitda=float(row[20]), profit_margin=float(row[21]), annualized_profit_margin=float(row[22]), held_percent_institutions=float(row[23]), forward_eps=float(row[24]), trailing_eps=float(row[25]), previous_close=float(row[26]), trailing_eps_percentage=float(row[27]), price_to_book=float(row[28]), shares_outstanding=float(row[29]), net_income_to_common_shareholders=float(row[30]), nitcsh_to_shares_outstanding=float(row[31]), num_employees=int(row[32]), enterprise_value=int(float(row[33])), nitcsh_to_num_employees=float(row[34]), earnings_quarterly_growth=float(row[35]), price_to_earnings_to_growth_ratio=float(row[36]), sqrt_peg_ratio=float(row[37]), annualized_cash_flow_from_operating_activities=float(row[38]), ev_to_cfo_ratio=float(row[39]), last_4_dividends_0=float(row[40]), last_4_dividends_1=float(row[41]), last_4_dividends_2=float(row[42]), last_4_dividends_3=float(row[43]))
