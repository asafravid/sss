#########################################################
# Version 225 - Author: Asaf Ravid <asaf.rvd@gmail.com> #
#########################################################
...
MARKET_CAP_TO_ANNUALIZED_REVENUE_FACTOR   = 1000.0 # When market cap is taken instead of EV (if EV is negative) - factor by 1000
    ...
    try:
        ...
        if build_csv_db:
            ...
            if 'priceToSalesTrailing12Months' in info and info['priceToSalesTrailing12Months'] is not None:
                ...
                if stock_data.annualized_revenue is not None and stock_data.annualized_revenue > 0:
                    stock_data.trailing_12months_price_to_sales  = MARKET_CAP_TO_ANNUALIZED_REVENUE_FACTOR * stock_data.market_cap / stock_data.annualized_revenue
                else:
        ...
        if build_csv_db:
            ...
            if stock_data.annualized_cash_flow_from_operating_activities >= 0:
                if stock_data.enterprise_value == 0: # When 0, it means either EV is 0 (strange!) or a very very good cash flow (strange as well)
    ...
    if research_mode: # Update the appearance counter using ssss and sssss
        ...
        if appearance_counter_min <= list_len_sss   <= appearance_counter_max:
            for index, row in enumerate(sorted_list_sss):
                appearance_counter_dict_sss[  (row[0],row[1],row[2],row[3],row[28])] = appearance_counter_dict_sss[  (row[0],row[1],row[2],row[3],row[28])]+math.sqrt(float(list_len_sss  -index))/float(list_len_sss  )
        if appearance_counter_min <= list_len_ssss  <= appearance_counter_max:
            for index, row in enumerate(sorted_list_ssss):
                appearance_counter_dict_ssss[ (row[0],row[1],row[2],row[4],row[28])] = appearance_counter_dict_ssss[ (row[0],row[1],row[2],row[4],row[28])]+math.sqrt(float(list_len_ssss -index))/float(list_len_ssss )
        ...
        if appearance_counter_min <= list_len_sssss <= appearance_counter_max:
            for index, row in enumerate(sorted_list_sssss):
                appearance_counter_dict_sssss[(row[0],row[1],row[2],row[5],row[28])] = appearance_counter_dict_sssss[(row[0],row[1],row[2],row[5],row[28])]+math.sqrt(float(list_len_sssss-index))/float(list_len_sssss)
