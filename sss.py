#########################################################
# Version 106 - Author: Asaf Ravid <asaf.rvd@gmail.com> #
#########################################################
...
def process_info(symbol, stock_data, build_csv_db_only, use_investpy, tase_mode, sectors_list, build_csv_db, profit_margin_limit, ev_to_cfo_ratio_limit, min_enterprise_value_millions_usd, earnings_quarterly_growth_min, earnings_quarterly_growth_unknown, enterprise_value_to_revenue_limit, favor_technology_sector, market_cap_included, research_mode):
    try:
        ...
        if stock_data.enterprise_value_to_revenue is not None: stock_data.evr_effective = stock_data.enterprise_value_to_revenue
        ...
        if stock_data.trailing_price_to_earnings is not None:
            if favor_technology_sector and stock_data.sector == 'Technology':
                stock_data.tpe_effective = stock_data.trailing_price_to_earnings / float(favor_technology_sector)  # ** 2
            else:
        ...
        if stock_data.ev_to_cfo_ratio is None  or stock_data.ev_to_cfo_ratio > ev_to_cfo_ratio_limit:
            if stock_data.ev_to_cfo_ratio is not None or stock_data.ev_to_cfo_ratio <= 0:
                if not build_csv_db_only and stock_data.ev_to_cfo_ratio > ev_to_cfo_ratio_limit:
        ...
        if build_csv_db:
            if return_value:
                ...
                if max_profit_margin_effective and earnings_qgrowth_factor_effective and stock_data.price_to_earnings_to_growth_ratio > 0:
                    ...
                    if stock_data.trailing_price_to_earnings is not None and stock_data.enterprise_value_to_ebitda is not None:
                        stock_data.ssss_value    = float(((stock_data.evr_effective * stock_data.tpe_effective * stock_data.enterprise_value_to_ebitda) / (max_profit_margin_effective                                       )) * ((stock_data.sqrt_peg_ratio * stock_data.ev_to_cfo_ratio) / earnings_qgrowth_factor_effective))  # the lower  the better
                        stock_data.sssss_value   = float(((stock_data.evr_effective * stock_data.tpe_effective * stock_data.enterprise_value_to_ebitda) / (max_profit_margin_effective                                       )) * ( stock_data.sqrt_peg_ratio                               / earnings_qgrowth_factor_effective))  # the lower  the better
                    ...
                    if stock_data.trailing_price_to_earnings is not None and stock_data.enterprise_value_to_ebitda is not None:
                        stock_data.ssssi_value   = float(((stock_data.evr_effective * stock_data.tpe_effective * stock_data.enterprise_value_to_ebitda) / (max_profit_margin_effective * stock_data.held_percent_institutions)) * ((stock_data.sqrt_peg_ratio * stock_data.ev_to_cfo_ratio) / earnings_qgrowth_factor_effective))  # the lower  the better
                        stock_data.sssssi_value  = float(((stock_data.evr_effective * stock_data.tpe_effective * stock_data.enterprise_value_to_ebitda) / (max_profit_margin_effective * stock_data.held_percent_institutions)) * ( stock_data.sqrt_peg_ratio                               / earnings_qgrowth_factor_effective))  # the lower  the better
        ...
        if return_value and not research_mode: print('                                          sector: {:15}, sss_value: {:15}, ssss_value: {:15}, sssss_value: {:15}, ssse_value: {:15}, sssse_value: {:15}, ssssse_value: {:15}, sssi_value: {:15}, ssssi_value: {:15}, sssssi_value: {:15}, sssei_value: {:15}, ssssei_value: {:15}, sssssei_value: {:15}, enterprise_value_to_revenue: {:15}, evr_effective: {:15}, trailing_price_to_earnings: {:15}, tpe_effective: {:15}, enterprise_value_to_ebitda: {:15}, profit_margin: {:15}, annualized_profit_margin: {:15}, held_percent_institutions: {:15}, forward_eps: {:15}, trailing_eps: {:15}, price_to_book: {:15}, shares_outstanding: {:15}, net_income_to_common_shareholders: {:15}, nitcsh_to_shares_outstanding: {:15}, num_employees: {:15}, enterprise_value: {:15}, nitcsh_to_num_employees: {:15}, earnings_quarterly_growth: {:15}, price_to_earnings_to_growth_ratio: {:15}, sqrt_peg_ratio: {:15}, annualized_cash_flow_from_operating_activities: {:15}, ev_to_cfo_ratio: {:15}'.format(stock_data.sector, stock_data.sss_value, stock_data.ssss_value, stock_data.sssss_value, stock_data.ssse_value, stock_data.sssse_value, stock_data.ssssse_value, stock_data.sssi_value, stock_data.ssssi_value, stock_data.sssssi_value, stock_data.sssei_value, stock_data.ssssei_value, stock_data.sssssei_value, stock_data.enterprise_value_to_revenue, stock_data.evr_effective, stock_data.trailing_price_to_earnings, stock_data.tpe_effective, stock_data.enterprise_value_to_ebitda, stock_data.profit_margin, stock_data.annualized_profit_margin, stock_data.held_percent_institutions, stock_data.forward_eps, stock_data.trailing_eps, stock_data.price_to_book, stock_data.shares_outstanding, stock_data.net_income_to_common_shareholders, stock_data.nitcsh_to_shares_outstanding, stock_data.num_employees, stock_data.enterprise_value, stock_data.nitcsh_to_num_employees, stock_data.earnings_quarterly_growth, stock_data.price_to_earnings_to_growth_ratio, stock_data.sqrt_peg_ratio, stock_data.annualized_cash_flow_from_operating_activities, stock_data.ev_to_cfo_ratio))
        return return_value
...
def process_symbols(symbols, csv_db_data, rows, rows_no_div, rows_only_div, thread_id, build_csv_db_only, use_investpy, tase_mode, sectors_list, build_csv_db, relaxed_access, profit_margin_limit, ev_to_cfo_ratio_limit, min_enterprise_value_millions_usd, earnings_quarterly_growth_min, earnings_quarterly_growth_unknown, enterprise_value_to_revenue_limit, favor_technology_sector, market_cap_included, research_mode):
    ...
    if build_csv_db:
        for symb in symbols:
            ...
            rows.append(                           [stock_data.ticker, stock_data.short_name, stock_data.sector, stock_data.sss_value, stock_data.ssss_value, stock_data.sssss_value, stock_data.ssse_value, stock_data.sssse_value, stock_data.ssssse_value, stock_data.sssi_value, stock_data.ssssi_value, stock_data.sssssi_value, stock_data.sssei_value, stock_data.ssssei_value, stock_data.ssssei_value, stock_data.enterprise_value_to_revenue, stock_data.evr_effective, stock_data.trailing_price_to_earnings, stock_data.tpe_effective, stock_data.enterprise_value_to_ebitda, stock_data.profit_margin, stock_data.annualized_profit_margin, stock_data.held_percent_institutions, stock_data.forward_eps, stock_data.trailing_eps, stock_data.price_to_book, stock_data.shares_outstanding, stock_data.net_income_to_common_shareholders, stock_data.nitcsh_to_shares_outstanding, stock_data.num_employees, stock_data.enterprise_value, stock_data.nitcsh_to_num_employees, stock_data.earnings_quarterly_growth, stock_data.price_to_earnings_to_growth_ratio, stock_data.sqrt_peg_ratio, stock_data.annualized_cash_flow_from_operating_activities, stock_data.ev_to_cfo_ratio, stock_data.last_4_dividends_0, stock_data.last_4_dividends_1, stock_data.last_4_dividends_2, stock_data.last_4_dividends_3])
            ...
            for fix_row_index in range(3,len(row)):  # for empty strings - convert value to 0
                if row[fix_row_index] == '':
                    if fix_row_index == 1:  # Name == '' --> 'None'
                        ...
                        row[fix_row_index] = 0
            stock_data = StockData(ticker=symbol, short_name=row[1], sector=row[2], sss_value=float(row[3]), ssss_value=float(row[4]), sssss_value=float(row[5]), ssse_value=float(row[6]), sssse_value=float(row[7]), ssssse_value=float(row[8]), sssi_value=float(row[9]), ssssi_value=float(row[10]), sssssi_value=float(row[11]), sssei_value=float(row[12]), ssssei_value=float(row[13]), sssssei_value=float(row[14]), enterprise_value_to_revenue=float(row[15]), evr_effective=float(row[16]), trailing_price_to_earnings=float(row[17]), tpe_effective=float(row[18]), enterprise_value_to_ebitda=float(row[19]), profit_margin=float(row[20]), annualized_profit_margin=float(row[21]), held_percent_institutions=float(row[22]), forward_eps=float(row[23]), trailing_eps=float(row[24]), price_to_book=float(row[25]), shares_outstanding=float(row[26]), net_income_to_common_shareholders=float(row[27]), nitcsh_to_shares_outstanding=float(row[28]), num_employees=int(row[29]), enterprise_value=int(float(row[30])), nitcsh_to_num_employees=float(row[31]), earnings_quarterly_growth=float(row[32]), price_to_earnings_to_growth_ratio=float(row[33]), sqrt_peg_ratio=float(row[34]), annualized_cash_flow_from_operating_activities=float(row[35]), ev_to_cfo_ratio=float(row[36]), last_4_dividends_0=float(row[37]), last_4_dividends_1=float(row[38]), last_4_dividends_2=float(row[39]), last_4_dividends_3=float(row[40]))
...
def sss_run(sectors_list, build_csv_db_only, build_csv_db, csv_db_path, read_united_states_input_symbols, tase_mode, num_threads, market_cap_included, use_investpy, research_mode, profit_margin_limit, ev_to_cfo_ratio_limit, min_enterprise_value_millions_usd, best_n_select, enterprise_value_to_revenue_limit, favor_technology_sector, generate_result_folders=1, appearance_counter_dict_ssss={}, appearance_counter_dict_sssss={}, appearance_counter_min=25, appearance_counter_max=35):
    ...
    elif favor_technology_sector:        sectors_str           = '_FTB{}'.format(favor_technology_sector)
